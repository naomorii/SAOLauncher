#!/usr/bin/env python3
import gi, os, functools, subprocess, json
gi.require_version("Gtk", "4.0")
gi.require_version("Gio", "2.0")
from gi.repository import Gtk, GLib, Gdk, Gio


class SAOLauncher(Gtk.Application):

    def __init__(self, triangle_width=20, triangle_height=30, button_width=160):
        super().__init__(application_id="io.github.naomorii.SAOLauncher")
        ...
        self.ICON_DIR = os.path.expanduser("~/.config/saolauncher/icons")

        # ---------- LOAD JSON CONFIG ----------
        config_dir = os.path.expanduser("~/.config/saolauncher")
        ...
        if not os.path.exists(config_path):
            ...
            try:
                ...
                print("Failed to load launcher_config.json:", e)
                config = {}

        self.menu_items = config.get("menu_items", {})
        self.commands = config.get("commands", {})
        self.ICON_MAP = config.get("icon_map", {})
        self.ICON_ACTIVE_MAP = config.get("icon_active_map", {})
    ...
    def create_icon_button(self, label, icon_only=False, floating=False):
        ...
        icon_hover_path = self.ICON_ACTIVE_MAP.get(label)
        ...
        if btn.img_hover:
            ...
            pointer.connect(
                "enter",
                lambda *args: self.fade_hover(btn.img_hover, 1.0)
            ...
            pointer.connect(
                "leave",
                lambda *args: self.fade_hover(btn.img_hover, 0.0)
                if not btn.is_active
                else None,
            )
    ...
    def set_button_icon_state(self, btn, state="normal"):
        ...
        if state == "active":
            ...
            if btn.img_hover:
                self.fade_hover(btn.img_hover, 0.0)

            icon_path = self.ICON_ACTIVE_MAP.get(
                btn.label_str, self.ICON_MAP.get(btn.label_str)
            )
            ...
            if icon_path and btn.img_base:
                ...
                if os.path.exists(full_path):
                    btn.img_base.set_filename(full_path)
    ...
    def handle_menu_click(self, btn, label, children, level, parent_btn, create_floating_menu):
        ...
        if label == "Close":
            ...
            def create_icon_btn(icon_name, callback):
                ...
                pic = Gtk.Picture.new_for_filename(
                    os.path.join(self.ICON_DIR, icon_name)
                )
        ...
        if label == "Config File":
            config_path = os.path.expanduser("~/.config/saolauncher/launcher_config.json")
        ...
        if label == "Icons":
            icons_path = os.path.expanduser("~/.config/saolauncher/icons")
            if os.path.exists(icons_path):
                subprocess.Popen(["xdg-open", icons_path])
                self.close_app_sequence()
            else:
                print(f"Config file not found: {icons_path}")
                config = {}
            return
